        ...
        if len(lines) > 1:
            ...
            if select_line_idx is not None:
                ...
                for idx in range(select_line_idx, len(lines)):
                    ...
                    if any(word in line.lower() for word in ['this query', 'explanation:', 'note:', 'the above']):
                        break
    ...
    def validate_sql_syntax(sql: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not sql_lower.startswith('select'):
                logger.warning("SQL validation failed: Query does not start with SELECT")
                return {"valid": False, "error": "Query must start with SELECT"}
            
            # Must contain FROM
            if 'from' not in sql_lower:
                logger.warning("SQL validation failed: Query does not contain FROM clause")
            ...
            for keyword in LLMConfig.DANGEROUS_KEYWORDS:
                if keyword in sql_lower:
                    logger.warning(f"SQL validation failed: Contains dangerous keyword '{keyword}'")
                    return {"valid": False, "error": f"Contains dangerous keyword: {keyword}"}
...
class ConfidenceScorer:
    ...
    @staticmethod
    def calculate_confidence(natural_language_query: str, sql: str, schema_info) -> float:
        """
