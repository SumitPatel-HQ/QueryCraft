...
async def query_database(database_id: int, request: QueryRequest):
    ...
    with get_db() as db:
        ...
        try:
            ...
            logger.info(f"📊 Schema tables available in database '{database.name}': {available_tables}")
            ...
            schema_hash = hashlib.md5(str(sorted(schema_data.keys())).encode()).hexdigest()
            if CACHE_ENABLED:
                cached_result = query_cache.get(request.question, schema_hash)
                if cached_result:
                    logger.info(f"⚡ CACHE HIT - Returning cached result for: {request.question}")
                    # Add execution time for cache hit
                    cached_result["execution_time_ms"] = int((time.time() - start_time) * 1000)
                    cached_result["generation_method"] = cached_result.get("generation_method", "cached")
            ...
            try:
                logger.info(f"Executing query on database: {database.file_path or database.connection_string}")
                ...
                if "no such table" in error_str.lower():
                    ...
                    raise HTTPException(
                        status_code=400, 
                        detail=f"SQL execution error: {error_str}"
                    )
            
            execution_time_ms = int((time.time() - start_time) * 1000)
            confidence = min(95, max(85, base_confidence + 35))
            complexity = determine_query_complexity(sql_query)
            why_explanation = generate_query_explanation(request.question, sql_query, tables_used, results)
...
async def get_database_tables_info(database_id: int):
    """
