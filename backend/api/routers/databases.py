...
async def list_databases():
    ...
    with get_db() as db:
        databases = db.query(DatabaseModel).filter(
            DatabaseModel.is_active == True
        ).order_by(DatabaseModel.last_accessed.desc()).all()
    ...
    try:
        ...
        if file_path.endswith('.db') or file_path.endswith('.sqlite'):
            ...
            if not DatabaseUploadHandler.validate_sqlite_file(file_path):
                raise HTTPException(status_code=400, detail="Invalid or empty SQLite database file")
        elif file_path.endswith('.sql'):
            # Import SQL dump to SQLite
            db_type = 'sqlite'
            try:
                original_path = file_path
                file_path = await DatabaseUploadHandler.import_sql_dump(file_path, db_name)
                ...
                if "syntax error" in error_msg.lower():
                    ...
                    raise HTTPException(status_code=400, detail=f"Failed to import SQL file: {error_msg[:200]}")
            ...
            try:
                original_path = file_path
                file_path = await DatabaseUploadHandler.import_csv(file_path, table_name, db_name)
                # Clean up the original .csv file
                if os.path.exists(original_path):
                    os.remove(original_path)
                temp_file_path = file_path
            except Exception as e:
                raise HTTPException(status_code=400, detail=f"Failed to import CSV file: {str(e)[:200]}")
        else:
...
async def get_database_schema(database_id: int):
...
async def get_database_tables(database_id: int):
